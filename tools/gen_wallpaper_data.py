#!/usr/bin/env python3
"""Regenerate kernel/wallpaper_data.c from assets/wallpapers/*.png.

The kernel has no PNG/DEFLATE decoder (and freestanding, ring-0 C is a bad
place to write one), so wallpaper pixel data is decoded once here, at build
time, on the host, and baked directly into a C source file as plain byte
arrays. The kernel just blits it - no runtime image decoding at all.

Requires macOS's `sips` (used only to downsample + convert PNG -> 24bpp BMP,
which stdlib alone can't do). If you're not on macOS, swap the sips call
below for an equivalent PNG resize/convert step from any image tool - the
BMP parsing and C-array emission below are pure stdlib and don't care how
the BMP was produced.
...
# Must match WALLPAPER_IMG_W/H in kernel/wallpaper_data.h. 100x75 divides the
# 800x600 desktop exactly (8x upscale, no fractional scaling).
IMG_W = 100
IMG_H = 75
...
def main():
    names = sorted(
        f[:-4] for f in os.listdir(SRC_DIR) if f.endswith(".png")
    )
    if not names:
    ...
    lines = [
        "/* Auto-generated from the PNGs in assets/wallpapers by",
        " * tools/gen_wallpaper_data.py - do not hand-edit. Re-run that",
        " * script after changing/adding wallpaper PNGs. */",
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for name in names:
            ...
            print(f"  {name}: wallpaper_{ident} {w}x{h} ({len(pixels)} bytes)")

    with open(OUT_PATH, "w") as f:
        f.write("\n".join(lines))
    print(f"wrote {OUT_PATH} ({len(names)} wallpapers)")
